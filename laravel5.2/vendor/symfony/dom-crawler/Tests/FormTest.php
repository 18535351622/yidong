...
class FormTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $formHtml = '<form id="login_form" action="https://sign.foo.com/sign.php?login_attempt=1" method="POST"><input type="text" name="foo" value="foo" /><input type="submit" /></form>';

        $form = $this->createForm($formHtml);
        $this->assertEquals('https://sign.foo.com/sign.php?login_attempt=1', $form->getUri(), '->getUri() returns absolute URIs set in the action form');

        $form = $this->createForm($formHtml, null, 'https://sign.foo.com');
        $this->assertEquals('https://sign.foo.com/sign.php?login_attempt=1', $form->getUri(), '->getUri() returns absolute URIs set in the action form');

        $form = $this->createForm($formHtml, null, 'https://sign.foo.com/bar/');
        $this->assertEquals('https://sign.foo.com/sign.php?login_attempt=1', $form->getUri(), '->getUri() returns absolute URIs set in the action form');
