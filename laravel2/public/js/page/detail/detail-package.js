...
//					$D.func.inAdvanceOrDelay(); //提前和延住
				...
				//客服的wechat和qq
				var _contient = $("#continent").val();
				$.ajax({
					url: "#/kefu?contient="+_contient,
					type: 'post',
					dataType: 'JSON',
					success: function(data){
						var content = '';
						if(data.rs == 1){
							content = ['<img src="'+data.data.wechatAndQQ.wcQRCode+'" alt="" width="80" height="80">',
							    '<span><img src="'+data.data.wechatAndQQ.photoImg+'" width="77" height="70"></span>',
								'<h3>'+data.data.wechatAndQQ.wcNichName+'</h3>',
								'<p>'+data.data.continentName+'旅行顾问</p><h4 class="font_color_orange">加我微信，沟通更快</h4>'
							].join('');
						}
						$("#wqConsultant").html(content);
					}
				});
			...
			loadCalendar: function(month) { //加载日历并切换相应band
				$.ajax({
					url: "#/xpack/line-calendar/"+$D.config.lineCode+"/" + month,
					dataType: "json",
					beforeSend: function() {
						$(".global_loading").show();
					},
...
//				if(car == undefined) car = 0;
				$.ajax({
					url: "#/xpack/caculate-price/"+$("#lineCode").val()+"/" + $("#start-date").val(),
					data: {
						ad: ad,
						kd: kd,
						hotel: hotel,
